          ...
          {basketItems && basketItems.map((item: BasketItem) => (
              <div key={item.id} className={classNames(classes.row, classes.item, classes.start)}>
                  ...
                  <div className={classes.column}>
                        <div className={classNames(classes.text, classes.name)}>{item.name}</div>
                        <div className={classNames(classes.text, classes.price)}>£{item.price.toFixed(2)}</div>
                  </div>
              </div>
          ))}
          <div className={classNames(classes.row, classes.background)}>
            <div className={classNames(classes.text, classes.total)}>Total: <span className={classes.black}>£{total.toFixed(2)}</span></div>
            <Button
                archetype="submit"
