...
func (l *companyLoader) loadManagerBatch(ctx context.Context, keys dataloader.Keys) []*dataloader.Result {
	n := len(keys)
	grant, authErr := middleware.Authenticate(ctx.Value("token").(string))
	if authErr != nil {
		return loadBatchError(authErr, n)
	}
	var managers middleware.CompanyToManagers
	managers, err := grant.GetManagersByCompany(keys.Keys())
	if err != nil {
		return loadBatchError(err, n)
	}

	if n != len(managers) {
		loadBatchError(&errors.ErrWhileHandling, n)
	}
	// Order them correctly
	var results = make([]*dataloader.Result, n)
	for i, key := range keys.Keys() {
		results[i] = &dataloader.Result{Data: managers[key]}
	}
	return results
}
...
// LoadManagersFromCompany loads a list of managers from a company uuid
func LoadManagersFromCompany(ctx context.Context, uuid string) ([]gentypes.Manager, error) {
	var managers []gentypes.Manager
	data, err := extractAndLoad(ctx, managerGroupLoaderKey, uuid)
	if err != nil {
		return managers, err
	}
	managers, ok := data.([]gentypes.Manager)
	if !ok {
		return managers, fmt.Errorf("Wrong type: %T", data)
	}
	return managers, nil
}
